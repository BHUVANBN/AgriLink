...
import { ShoppingCart, Check, ArrowRight, Eye, EyeOff, Shield, TrendingUp, FileText } from 'lucide-react';
...
export default function RegisterFarmer() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const body = { fullName: form.name, email: form.email, phone: form.phone, password: form.password };
    ...
    <div className="min-h-screen bg-brand-bg font-sans flex flex-col selection:bg-brand-green/20">
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6">
        <motion.div 
          ...
          className="bg-white rounded-2xl p-12 border border-[#eae6de] shadow-[0_12px_40px_rgba(0,0,0,0.08)] max-w-[480px] w-full relative overflow-hidden"
        >
          ...
          {step === 'form' ? (
            <>
              <div className="text-center mb-10 relative z-10">
                ...
                <h2 className="text-[28px] font-serif text-text-dark mb-3 font-semibold">Create Farmer Account</h2>
                <p className="text-[#69665f] text-sm leading-relaxed">
                  Enter your details to join the AgriLink community and start your smart farming journey.
                </p>
              </div>

              <form onSubmit={handleRegister} className="space-y-5 relative z-10">
                <div>
                  <label className="block text-sm font-serif text-text-dark mb-2 ml-1 font-medium">Full Name</label>
                  <input
                ...
                <div>
                  <label className="block text-sm font-serif text-text-dark mb-2 ml-1 font-medium">Email</label>
                  <input 
                    className="input-field !py-3.5 !text-base" 
                    type="email" 
                    placeholder="ramesh.kumar@example.in" 
                    value={form.email} 
                ...
                <div>
                  <label className="block text-sm font-serif text-text-dark mb-2 ml-1 font-medium">Phone Number</label>
                  <input 
                    className="input-field !py-3.5 !text-base" 
                    type="tel" 
                ...
                <div className="relative">
                  <label className="block text-sm font-serif text-text-dark mb-2 ml-1 font-medium">Password</label>
                  <div className="relative">
                    <input 
                      className="input-field !py-3.5 !text-base pr-12" 
                ...
                <div className="pt-4">
                  <button type="submit" disabled={loading} className="btn-primary w-full !py-4 !text-base !rounded-lg shadow-lg hover:shadow-xl transition-all duration-300">
                    {loading ? 'Creating Account...' : 'Register as Farmer'}
                  </button>
                </div>
              </form>

              <p className="text-[#69665f] text-sm text-center mt-8 relative z-10">
                Already have an account? <Link href="/auth/login" className="text-brand-green font-medium hover:underline transition-colors">Log in</Link>
              </p>
            </>
          ) : (
            <>
              <div className="text-center mb-10 relative z-10">
                ...
                <h2 className="text-[28px] font-serif text-text-dark mb-3 font-semibold">Check Email</h2>
                <p className="text-[#69665f] text-sm leading-relaxed">
              ...
              <form onSubmit={handleVerifyOtp} className="space-y-8 relative z-10">
                <div>
                  <input
                    className={`w-full bg-transparent border-b-2 text-center text-3xl font-medium tracking-[0.4em] outline-none transition-all pb-3 ${otpError ? 'border-red-400 text-red-900' : 'border-[#dfdcd5] text-text-dark focus:border-brand-green'}`}
                    placeholder="000000"
                    value={otp}
                    ...
                    required
                  />
                  {otpError && <p className="text-red-500 text-sm font-medium mt-3 text-center">{otpError}</p>}
                </div>

                <button type="submit" disabled={loading || otp.length !== 6} className="btn-primary w-full !py-4 !text-base !rounded-lg shadow-lg hover:shadow-xl transition-all duration-300">
                  {loading ? 'Verifying...' : 'Confirm Identity'}
                </button>

                <div className="text-center pt-2">
                  <button type="button" className="text-text-muted text-sm hover:text-text-dark transition-colors">
